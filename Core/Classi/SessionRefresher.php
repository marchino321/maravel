...
class SessionRefresher
{
    ...
    public function refresh(): void
    {
        ...
        if (!isset($_SESSION[$userIdKey])) {
            Debug::log("Nessuna sessione da aggiornare.", "SISTEMA");
            return;
        }
        ...
        if (!empty($userData[0])) {
           ...
           // $_SESSION['DefaultSetting']  = $data['DefaultSetting'] ?? $_SESSION['DefaultSetting'];
            ...
            Debug::log("Sessione utente {$userId} aggiornata con successo", "SISTEMA");
        } else {
            Debug::log("Impossibile aggiornare sessione: utente {$userId} non trovato", "SISTEMA");
        }
    }
}
